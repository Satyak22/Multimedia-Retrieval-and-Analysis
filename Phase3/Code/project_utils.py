...
def plot_results(similar_images, query_image_path):
    ...
    for x in similar_images:
        if k <= 0:
            break
        s_arr[i][j].axis("off")
        s_arr[i][j].text(0.5,-0.1,x["imageName"], size=6, ha="center", transform=s_arr[i][j].transAxes)
...
def dimensionality_reduction(data_matrix, image_ids, args, label=None, viz=False):
    if args.lsa_model == "SVD":
        ...
        if args.model == "CM":
            ...
            reduced_data = reduce_dimensions_lda(data_matrix_cmlda, args.k_features)
        else:
...
def get_V_matrix(data_matrix, image_ids, args, label=None, viz=False):
    if args.lsa_model == "SVD":
        reduced_data, V_matrix = reduce_dimensions_svd(data_matrix, args.k_features,  get_v=True)
    elif args.lsa_model == "PCA":
        reduced_data, V_matrix = reduce_dimensions_pca(data_matrix, args.k_features, get_v=True)
    elif args.lsa_model == "NMF":
        if args.model == "CM":
            print("NMF is not applicable to Color Moments")
            return
        reduced_data, V_matrix = reduce_dimensions_nmf(data_matrix, args.k_features, get_v=True)
    elif args.lsa_model == "LDA":
        if args.model == "CM":
            ...
            reduced_data, V_matrix = reduce_dimensions_lda(data_matrix, args.k_features, get_v=True)
...
def get_actual_labels(image_ids, db_handle):
    labels = []
    for image_id in image_ids:
        image_info = list(db_handle.mwdb_project.image_features.find({"imageName" : image_id}))[0]
        if "dorsal" in image_info["aspectOfHand"]:
            labels.append((image_id, "dorsal"))
        else:
            labels.append((image_id, "palmar"))
    return labels

def get_actual_labels_from_csv(csv_path, test_image_ids):
    image_ids = [ img.split('/')[-1] for img in test_image_ids ]
    labels = []
    df = pd.read_csv(csv_path)
    rows = df.loc[df['imageName'].isin(image_ids)]
    for index, row in rows.iterrows():
        if "dorsal" in row["aspectOfHand"]:
            label = "dorsal"
        else:
            label = "palmar"
        labels.append((row['imageName'], label))
    return labels

def get_target_matrix(image_ids, model, db_handle):
    labels = []
    data = []
    for image_id in image_ids:
        print(image_id)
        image_info = list(db_handle.mwdb_project.image_features.find({"imageName" : image_id}))[0]
        data.append(np.array(image_info[model]))
    return np.array(data), image_ids

def get_accuracy(predicted, actual):
    count = 0
    sorted_predict = sorted(predicted, key = lambda x: x[0], reverse = True)
    sorted_actual = sorted(actual, key = lambda x: x[0], reverse = True)

    for predict, real in zip(sorted_predict, sorted_actual):
        if predict[0] == real[0] and predict[1] == real[1]:
            count += 1

    return (count / len(sorted_predict)) * 100
